    ...
    post 'Create a user' do
      ...
      response '201', 'OK' do
        ...
        run_test! do |response|
          ...
          expect(parsed_response.keys).to(eq(%w[id email company_id role created_at updated_at jti]))
    ...
    post 'User logs in' do
      ...
      response '200', 'OK' do
        unless User.find_by(email: 'test@example.com')
          FactoryBot.create(:employee, email: 'test@example.com',
                                       password: 'abc123.')
        end
        ...
        run_test! do |response|
          ...
          decoded_token = JWT.decode(parsed_response['token'], ENV['devise_jwt_secret_key'], true,
